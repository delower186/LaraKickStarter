                        ...
                        @if ($ccategories->isNotEmpty())
                            @foreach ($ccategories as $ccategory)
                                <tr class="border-b border-gray-200 dark:border-gray-700">
                                    ...
                                    <td class="px-6 py-4">
                                        @can($permission->format('edit', 'ccategory'))
                                            <flux:button size="sm" variant="primary" icon="pencil-square" wire:navigate href="{{ route('ccategories.edit', $ccategory->id) }}"></flux:button>
                                        @endcan
                                        @can($permission->format('delete', 'ccategory'))
                                            <flux:button size="sm" variant="danger" icon="trash" wire:click="confirm({{ $ccategory->id }})"></flux:button>
                                        @endcan
                                    </td>
                                </tr>
                            @endforeach
